...
int Ro(const char *str, int *shift) {
    ...
    for (int i = 0; i < 13; ++i) {
        int lim = 0;
        while (lim < 3) {
            if (strlen(symbols[i]) == 1 && str[ind] == symbols[i][0]) {
                x += value[i];
                ind++;
            } else if (strlen(symbols[i]) == 2 && str[ind] == symbols[i][0] && str[ind + 1] == symbols[i][1]) {
                x += value[i];
                ind += 2;
            } else {
                break;
...
int Cv(const char *str, int base, int *shift, int *flag, char mod) {
    ...
    *shift = end_ptr - temp_str; // Вычисляем сдвиг
    free(temp_str);
    return (int)x;
}

void del_space(const char *source, char *dest, int len) {
    int idx = 0;
    for (int i = 0; i < len; ++i) {
        if (!isspace(source[i])) {
            dest[idx++] = source[i];
...
int overfscanf(FILE *stream, const char *format, ...) {
    ...
    for (int i = 0; i < len_format; ++i) {
        if (new_format[i] == '%' && i < len_format - 2) {
            if (new_format[i + 1] == 'R' && new_format[i + 2] == 'o') {
                while (isspace(c)) {
                    c = getc(stream);
                }
                ...
                *x = Ro(buffer, &shift);
                ...
                i += 2;
                free(buffer);
            } else if (new_format[i + 1] == 'Z' && new_format[i + 2] == 'r') {
                while (isspace(c)) {
                    c = getc(stream);
                }
                ...
                *x = Zr(buffer, &shift);
                ...
                i += 2;
                free(buffer);
            } else if (new_format[i + 1] == 'C' && (new_format[i + 2] == 'v' || new_format[i + 2] == 'V')) {
                while (isspace(c)) {
                    c = getc(stream);
                }
                int *x = va_arg(args, int *);
...
int main()
{
    ...
    puts("< Test_4 >");
    puts("String: '194394'");
    count = oversscanf("194394", "%Cv", &a, 100);
