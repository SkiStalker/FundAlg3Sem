...
callback read_from_file(FILE * filename, Node ** root, const char * separator)
{
...
callback collect_nodes(Node* node, Node*** nodes, int* len, int* capacity)
{
    ...
    while (!flag_stop)
    {
        ...
        switch (choice)
        {
            ...
            case '2':
                ...
                if (collect_nodes(root, &array, &len, &capacity) != SUCCESS)
                {
                    free(array);
                    free(separator);
                    delete_tree(root);
